...
public class PicPullRefreshListView extends ListView implements OnScrollListener {
	private int pageItemcount=ListPageConfig.picPageItemCount;
	...
	public PicPullRefreshListView(Context context) {
		super(context);
		mContext=context;
		init();
	}

	public PicPullRefreshListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext=context;
		init();
	}
...
//判断是否是一页中的最后一项且 第一页不用加载
		if(firstVisibleItem+visibleItemCount==totalItemCount && totalItemCount>=(18+1))
		{
			isLastRow=true;
		}
	}
